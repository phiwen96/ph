CC := clang++
CC_FLAGS= -std=c++2a -stdlib=libc++ -fmodules-ts -fmodules -fbuiltin-module-map -fimplicit-modules -fimplicit-module-maps -fprebuilt-module-path=.
AR = ar
AR_FLAGS = rv
...
# $(MODULES): %.pcm: %.cpp
# 	$(CC) -c $(FLAGS) $< -Xclang -emit-module-interface -o $@

$(APP).o: $(APP).cpp $(LIB).pcm
	$(CC) $(CC_FLAGS) -fmodule-file=$(LIB).pcm -c $< -o $@

# $(LIB).pcm: $(LIB).pcm(Number.pcm)

$(LIB).pcm: $(LIB).cpp Number.pcm 
 	$(CC) $(FLAGS) -fmodule-file=Number.pcm -c $< -Xclang -emit-module-interface -o $@

Number.pcm: Number.cpp Signed.pcm Unsigned.pcm Integer.pcm Float.pcm Convertible_to.pcm
	$(CC) $(CC_FLAGS) -fmodule-file=Integer.pcm -fmodule-file=Float.pcm -fmodule-file=Signed.pcm -fmodule-file=Unsigned.pcm -fmodule-file=Convertible_to.pcm -c $< -Xclang -emit-module-interface -o $@

Signed.pcm: Signed.cpp Convertible_to.pcm 
	$(CC) $(CC_FLAGS) -fmodule-file=Convertible_to.pcm -c $< -Xclang -emit-module-interface -o $@
...
Convertible_to.pcm: Convertible_to.cpp
	$(CC) $(CC_FLAGS) -c $< -Xclang -emit-module-interface -o $@
...
# docs: Ph.pdf Lib.png Lib.pdf
# #Lib.ps


# Ph.pdf: README.md
# 	pandoc --filter pandoc-plot --filter pandoc-plantuml -o Ph.pdf README.yml README.md


# # sips -s format jpeg project_dependencies.pdf --out project_dependencies.jpg

# # Lib.jpg: Lib.ps 
# # 	gs -sDEVICE=jpeg -dJPEGQ=100 -dNOPAUSE -dBATCH -dSAFER -r300 -sOutputFile=$@ $<
...
# Function.pcm: Function.cpp Same_as.pcm Convertible_to.pcm Typelist.pcm Assert.pcm
# 	$(CC) $(FLAGS) -fmodule-file=Same_as.pcm -fmodule-file=Convertible_to.pcm -fmodule-file=Typelist.pcm -fmodule-file=Assert.pcm -c $< -Xclang -emit-module-interface -o $@
...
# Lambda.pcm: Lambda.cpp Typelist.pcm
# 	$(CC) $(FLAGS) -fmodule-file=Typelist.pcm -c $< -Xclang -emit-module-interface -o $@
...
# Front.pcm: Front.cpp Same_as.pcm Assert.pcm
# 	$(CC) $(FLAGS) -fmodule-file=Same_as.pcm -fmodule-file=Assert.pcm -c $< -Xclang -emit-module-interface -o $@

# Back.pcm: Back.cpp Same_as.pcm Assert.pcm
# 	$(CC) $(FLAGS) -fmodule-file=Same_as.pcm -fmodule-file=Assert.pcm -c $< -Xclang -emit-module-interface -o $@
